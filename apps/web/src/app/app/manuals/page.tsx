...
export default function ManualsPage() {
  const { data: allObjects, loading, error } = useCacheObjects({ priority: "P1", status: "ACTIVE" });

  // Client-side: prefer objects tagged "manual" or "runbook"; fall back to all P1
  const objects = allObjects
    ? allObjects.filter(o => o.tags.some(t => ["manual", "runbook", "procedure", "sop"].includes(t.toLowerCase())))
        .length > 0
      ? allObjects.filter(o => o.tags.some(t => ["manual", "runbook", "procedure", "sop"].includes(t.toLowerCase())))
      : allObjects
    : [];
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Manuals</h1>
      <p className="text-gray-500 text-sm mb-6">Standard operating procedures, runbooks, and reference manuals.</p>
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="space-y-3">
          {objects.length ? objects.map(obj => (
            ...
            <div className="bg-white rounded-lg shadow p-8 text-center text-gray-400 text-sm">No manual content cached.</div>
          )}
        </div>
      )}
    </div>
