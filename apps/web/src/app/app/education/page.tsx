...
export default function EducationPage() {
  const { data: objects, loading, error } = useCacheObjects({ priority: "P2", status: "ACTIVE" });

  return (
    <div>
      <span className="inline-block text-xs font-medium px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-800 mb-4">P2 — Retained</span>
      <h1 className="text-2xl font-bold text-gray-900 mb-2">Education</h1>
      <p className="text-gray-500 text-sm mb-6">Training materials, language resources, and scientific references.</p>
      ...
      {loading ? <LoadingSpinner /> : (
        <div className="space-y-3">
          {objects?.length ? objects.map(obj => (
            ...
            <div className="bg-white rounded-lg shadow p-8 text-center text-gray-400 text-sm">No education content cached.</div>
          )}
        </div>
      )}
    </div>
